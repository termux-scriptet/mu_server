...
const vcard = 'BEGIN:VCARD\n'  //Jangan diganti,Ntar error
            ...
            + 'TEL;type=CELL;type=VOICE;waid=6282135890584:+6282135890584\n'  // Ganti jadi nomormu, tapi jangan ubah polanya
            + 'END:VCARD' // Jangan diganti,Ntar Error
            
//settings            
prefix = '.'
blocked = []   
limitawal = '100' //Terserah Mo Ganti apa kgk
memberlimit = 1 //Terserah Mo Ganti apa kgk
cr = '*Akira Bot By Aditia*'
vr = '*Akira Bot Wa*\n*Follow Ig aditiadit747*'

//owner number
const ownerNumber = ["6282135890584@s.whatsapp.net"]  //Ganti Jadi Nomormu
...
const akira = new WAConnection()
   ...
   console.log(color('[','aqua'),color('AkiRa','white'),color(']','aqua'),color('SQAN QR CODE DI WHATSAPP WEB!!','aqua'),color('You','white'),color('Tube','red'),color('Adit_YT','yellow'))
}) 

akira.on('credentials-updated', () => {
	const authInfo = akira.base64EncodedAuthInfo()
   console.log(`credentials updated!`)
   fs.writeFileSync('./lolteam.json', JSON.stringify(authInfo, null, '\t'))
})
fs.existsSync('./lolteam.json') && akira.loadAuthInfo('./lolteam.json')
akira.connect();
...
//CASE BADWORD
		...
		if (isBanned) return reply(ind.baned())
				...
				akira.sendMessage(from, anu, image, { quoted: mek, caption: 'Nih Hasilnya...' })
				break
				case 'hekelserti': 				
				// Subscribe Yoo
				if (!isRegistered) return reply(ind.noregis())
...
break		     		
		...
		if (isBanned) return reply(ind.baned())
					...
					txt = body.slice(10)
					reply(ind.wait())
					anu = await getBuffer(`https://api.vhtear.com/water_maker?text=${txt}&apikey=AkiraGithub`)
					akira.sendMessage(from, anu, image, { quoted: mek, caption: 'Nih Hasilnyaaa' })
					await limitAdd(sender)
					break
